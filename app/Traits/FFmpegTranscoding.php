...
trait FFmpegTranscoding
{
    ...
    public function transcodingToHLS($tempPathVideo, $resolution, $path_upload, $name, $message, $tmdb_id)
    {
		...
		\Log::info('2. transcodingToHLS > Create directory successfully.');
		...
		\Log::info('3. transcodingToHLS > Start converting.');
		...
		\Log::info('4. transcodingToHLS > End converting.');

        foreach ($resolution as $key => $value) {
			\Log::info('transcodingToHLS > Resolution.');
		...
		\Log::info('5. transcodingToHLS > going to Save movie.');        
        \Log::info('trans user', [shell_exec('who')]);
        $convert->save($path_upload . '/nonenc/' . $name . '.m3u8');
		\Log::info('6. transcodingToHLS > Save movie.');
    ...
    public function transcodeVideoToMp4($resolution, $id, $path, $tmdb_id, $cloud, $item_name)
    {
        ...
        // Check Cloud Disl
        if ($cloud === 'aws') {
            $output_path = '/movies/' . $item_name;
            $cloud_name = 's3_private';
        } else {
            $output_path = '/storage/movies/' . $item_name;
            $cloud_name = 'public';
        }
